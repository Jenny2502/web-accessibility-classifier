...
When training, the script prints the most frequent high-severity violation types.  
When predicting, `--high-from-csv` or `--high-from-json` is optional and only used to mark which predictions are high-severity.
...
- Predictions are best used for triage. Confirm issues with a full scanner (axe, pa11y).
